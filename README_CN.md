# 2.0已引入
在src/NewLeonardoTaiko中，我尝试使用一种新的防抖算法。希望能实现良好防抖效果的同时，提供更加优秀的连打性能。可能存在一些不稳定，需要对参数进行微调。
...
在NewLeonardoTaiko中，尝试引入了一种新的动态阈值方法。

在ArduinoLeonardo的实际采样中，敲击鼓体后，读取到的传感器值实际上是有一个逐渐递减的趋势的值，而并非正负晃动的值，为此，我们可以创建一个滑动窗口。在窗口内，以最晚进入窗口的值为基准，对阈值进行动态变更，来判断最新进入窗口的值是否为敲击触发的值。

实际操作中，对于四个通道的阈值分别进行阈值变化会比较麻烦，因此我们使用滑动窗口内最新的值减去最旧的值，可以得到一个统一的变化值。

此时，由于敲击后读取到的传感器值一般为递减趋势，在进行滑动窗口处理后，所得的值如果为突然增大的正数，则说明该传感器正在被敲击。如果为负数，则说明该传感器的值正在减少，说明没有被敲击。

利用这个特征，我们可以通过设定和调节相关参数，实现防抖动目的。
...
### ```cd_length = 10```
防串音检测时缓存区循环读取```analogValue```的循环次数。设置的越低，敲击延迟越低，但是过低的缓存区可能会导致缓存区无法读取到真正的最高值。
...
触发敲击，执行防串音检测的阈值。由于使用了INTERNAL的1.1V作为参考电压，信号会比默认5V更加明显，因此触发阈值会比旧版本中的数值高。
...
```break_check_limit```用于描述敲击后信号变化的窗口，```reset_threshold = 8```用于描述信号变化值应能被

敲击后，我们将被敲击的按键锁住，并执行防抖算法。由于信号本身存在抖动，在防抖动检测时，仅仅是读取到变化值为负数并不足以说明该传感器未被敲击，因此引入```break_check_limit```。

当传感器变化值大于```reset_threshold```时，说明信号仍然没有处于持续下降的趋势。当传感器变化值小于```reset_threshold```时，会持续读取变化值，如果读取到```break_check_limit```设定的个数的变化值均小于```reset_threshold```，表明这一段的传感器信号读数的趋势为下降，说明没有被敲击，因此可以放开，防抖结束。
